CREATE DEFINER=`root`@`%` PROCEDURE `PGSSALDOSREP`(
Par_Tipo CHAR,
...
TerminaStore: BEGIN
				...
				WHEN Instrumento_Cliente THEN 
					BEGIN 
						INSERT INTO lista_creditos
							SELECT CreditoID 
							FROM CREDITOS 
							WHERE Estatus NOT IN ('P','K','C') AND ClienteID=Par_OrigenID;
					END;
				WHEN Instrumento_Credito THEN 
					BEGIN 
						INSERT INTO lista_creditos
							SELECT CreditoID 
							FROM CREDITOS 
							WHERE Estatus NOT IN ('P','K','C') AND CreditoID=Par_OrigenID;
					END;
			ELSE    
					BEGIN
	...
	SET gc.FULTPACAP=coalesce(fecha_capital,FCOMPRA),
		...
		IF(Par_Tipo=Reporte_Global)THEN
			BEGIN
				SELECT '' as FOLIOUPD, 				'SAFI' as SIORIREQ, 			'SAFI' USRORIREQ , 
				'' IDELEMENTO,							'' IDELEPSSA,					IDPDTO,
				'' ORIPDTO,							PLAZOMAX,						IDPROGSSA,
				CTAPROGSSA,							SERIEPGSSA,						TDASUCPGSA,
				TPOCTAPGSA,							LIMITE,							MONTODIS,
				SDOCTA,								VDO,							COALESCE(VDO30,0)VDO30,
				COALESCE(VDO60,0)VDO60,				COALESCE(VDO90,0)VDO90,			COALESCE(VDOM90,0)VDOM90,
				IFIN,								IMOR,							DIASVDO, 
				...
				coalesce(PAGOMES18,0)PAGOMES18,		FULTPACAP,						FULTPAINT,
				ci.FechaIncumplimiento FPRIMINCUM,	gc.CreditoID PRESTAMOID, 		now()FEHODATOS, 
				c.RFC,								c.CURP ,						CAST(LINEACREDITO AS CHAR)LINEACREDITO, 
				CTAPROGSSA CTAPROGSSA2,							8 as NEGOCIOPGSSA, 				FCOMPRA,
				PAGOSVENCIDOS,						gc.NPARCIALIDAD,				PLAZO,
				TASA,								ESTATUS
				FROM 
				saldo_pago_mes spm  
