...
try:
    ...
    logger_service.error(f"Another instance of {script_name} is already running")
    sys.exit(1)
...
while True:
    ...
    if USR1_KILL_SIGNAL_SET:
        ...
        # Keep waiting
        logger_service.info('Waiting')
        print('Waiting')
        continue

    try:
 
        logger_service.info('Execute main')
        main()
        print('executed')
        logger_service.info('Main Executed')

    except Exception as e:
        logger_service.info(e)
        ...
        logger_service.debug(f"Waiting {DEBUG_SLEEP_TIME} seconds...")